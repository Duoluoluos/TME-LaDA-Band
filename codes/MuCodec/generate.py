...
def _resolve_auxiliary_asset(filename):
    """Prefer a bundled asset, then the public pretrained-root layout."""
    code_root = os.path.dirname(os.path.abspath(__file__))
    bundled_path = os.path.join(code_root, "tools", filename)
    if os.path.isfile(bundled_path):
        return bundled_path

    pretrained_root = os.environ.get("LADA_PRETRAINED_ROOT")
    if pretrained_root:
        public_path = os.path.join(pretrained_root, "mucodec", filename)
...
class MuCodec:
    def __init__(self, \
        ...
        if load_main_model:
            audio_ldm_path = _resolve_auxiliary_asset("audioldm_48k.pth")
    ...
    def _prepare_audio_batch(self, orig_samples):
        if orig_samples.ndim == 2:
            audios = orig_samples.unsqueeze(0)
        elif orig_samples.ndim == 3:
            audios = orig_samples
        else:
            raise AssertionError(orig_samples.shape)

        if audios.shape[1] == 1:
            audios = torch.cat([audios, audios], dim=1)
        elif audios.shape[1] != 2:
            raise ValueError(f"MuCodec expects 1 or 2 channels, got {audios.shape}")

        audios = audios.to(device=self.device, dtype=torch.float32)
        audios = torch.nan_to_num(audios, nan=0.0, posinf=1.0, neginf=-1.0)
        audios = torch.clamp(audios, min=-1.0, max=1.0).contiguous()
        return self.preprocess_audio(audios)

    def _encode_audio_windows(self, audios, batch_size):
        batch_count = audios.shape[0]
        orig_length = audios.shape[-1]
        min_samples = int(40.96 * self.sample_rate)
        window_samples = min_samples + 480
        output_len = int(orig_length / float(self.sample_rate) * 25) + 1

        while audios.shape[-1] < window_samples:
            audios = torch.cat([audios, audios], -1)

        num_segments = audios.shape[-1] // min_samples + 1
        audios = torch.cat([audios, audios], -1)
        audios = audios[:, :, : num_segments * window_samples]

        audio_input = (
            audios.reshape(batch_count, 2, num_segments, window_samples)
            .permute(0, 2, 1, 3)
            .reshape(batch_count * num_segments, 2, window_samples)
        )
        ...
        for audio_inx in range(0, audio_input.shape[0], batch_size):
            chunk = audio_input[audio_inx:audio_inx + batch_size]
            codes, _, _ = self.model.fetch_codes_batch(chunk, additional_feats=[], layer=self.layer_num)
            codes_list.append(torch.cat(codes, 1))

        codes = torch.cat(codes_list, 0)
        codes = (
            codes.reshape(batch_count, num_segments, codes.shape[1], codes.shape[2])
            .permute(0, 2, 1, 3)
            .reshape(batch_count, 1, -1)
        )
        return codes[:, :, :output_len]

    @torch.no_grad()
    @torch.autocast(device_type="cuda", dtype=torch.float32)
    def sound2code(self, orig_samples, batch_size=3):
        audios = self._prepare_audio_batch(orig_samples)
        return self._encode_audio_windows(audios, batch_size=batch_size)

    @torch.no_grad()
    @torch.autocast(device_type="cuda", dtype=torch.float32)
    def sound2code_batch(self, orig_samples, batch_size=None):
        audios = self._prepare_audio_batch(orig_samples)
        return self._encode_audio_windows(audios, batch_size=batch_size)
