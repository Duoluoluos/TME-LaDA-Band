...
def _resolve_muq_checkpoint(code_root):
    bundled_path = os.path.join(code_root, "muq_dev", "muq.pt")
    if os.path.isfile(bundled_path):
        return bundled_path

    pretrained_root = os.environ.get("LADA_PRETRAINED_ROOT")
    if pretrained_root:
        public_path = os.path.join(pretrained_root, "mucodec", "muq.pt")
...
class PromptCondAudioDiffusion(nn.Module):
    def __init__(
        ...
        _muq_dev_root = os.path.dirname(os.path.abspath(__file__))
        muencoder_dir = os.path.join(_muq_dev_root, "muq_dev/muq_fairseq")
        muencoder_ckpt = _resolve_muq_checkpoint(_muq_dev_root)

        self.muencoder = load_model(
            model_dir=muencoder_dir,
            checkpoint_dir=muencoder_ckpt,
        )
    ...
    def inference_codes(self, codes, spk_embeds, true_latents, latent_length,incontext_length, additional_feats, 
        ...
        device = quantized_muencoder_emb.device
