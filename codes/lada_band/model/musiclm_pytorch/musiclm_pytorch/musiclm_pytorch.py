...
from transformers import AutoTokenizer,AutoModelForMaskedLM,Wav2Vec2FeatureExtractor,AutoModel,XLMRobertaModel,AutoConfig
...
class SoftmaxContrastiveLearning(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.gather_fn = gather_fn if gather_fn is not None else lambda x: x
    ...
    def forward(self, audio_latents, text_latents):
        ...
        # audio_latents = self.gather_fn(audio_latents)
        # text_latents = self.gather_fn(text_latents)
        ...
        cosine_sims_exp = sims.exp() #[Rank, N, N]的相似度矩阵

        numerator = matrix_diag(cosine_sims_exp) #取对角线元素,也就是对于 t[l, i, j], 取所有i==j的元素, 得到一个 l*min(i,j) 的一维数组

        if self.decoupled_contrastive_learning:
            eye = torch.eye(batch, device = self.device, dtype = torch.bool)
            cosine_sims_exp = cosine_sims_exp.masked_fill(eye, 0.) #把对角线置为0
    ...
    def forward(self, audio_latents, text_latents):
        ...
        contrastive_loss = reduce(contrastive_loss, 'n -> 1', 'mean')
...
class SigmoidContrastiveLearning(nn.Module):
    ...
    def forward(self, audio_latents, text_latents):
        ...
        if audio_latents.ndim == 2:
            audio_latents = rearrange(audio_latents, '... -> 1 ...') #维度为: [Rank, Batch, Latent]
        ...
        sims = einsum('l i d, l j d -> l i j', audio_latents, text_latents) #两两之间计算点积相似度
        ...
        if exists(rank_sizes):
            ...
            labels = labels_by_ranks[dist.get_rank()] #获取当前rank的n个元素对应的标签
...
class AudioSpectrogramTransformerPretrained(nn.Module):
    ...
    def __init__(
        ...
        self.dim = dim #这个就是输出的dim
        self.sr = sr
        self.use_layer_idx = use_layer_idx #使用哪一层的特征


        conf = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
        # 强制注入 transformers >= 4.47 所需的新参数
        if not hasattr(conf, "conv_pos_batch_norm"):
            conf.conv_pos_batch_norm = False  # 设置为 False 以保持旧版行为

        self.model = AutoModel.from_pretrained(model_name, config=conf, trust_remote_code=True)
    ...
    def forward(
        ...
        layer_hidden_states = outputs.hidden_states[self.use_layer_idx]
...
class TextTransformerPretrained(nn.Module):
    ...
    def __init__(
        ...
        self.model = XLMRobertaModel.from_pretrained(model_name, trust_remote_code=True)
    ...
    def forward(
        ...
        with torch.no_grad():
            if exists(raw_texts):
                x = self.tokenizer(raw_texts, padding=True)
                x = torch.tensor(x['input_ids']).to(self.device)
...
class MuLaN(nn.Module):
    ...
    def __init__(
        ...
        klass = SigmoidContrastiveLearning if sigmoid_contrastive_loss else partial(SoftmaxContrastiveLearning, decoupled_contrastive_learning = decoupled_contrastive_learning, gather_fn = gather_fn)
        self.contrast = klass()

        self.multi_layer_contrastive_learning = None

        # 如果使用分层对比学习，则对每层的embedding进行对比
        if hierarchical_contrastive_loss:
            num_layers = default(hierarchical_contrastive_loss_layers, min(audio_transformer.depth, text_transformer.depth) - 1) #需要进行分层对比学习的层数
            assert num_layers > 0

            #使用register_buffer以将张量加入模型参数
            self.register_buffer('text_layers_indices', interspersed_indices(num_layers, text_transformer.depth)) #就是一个交叉索引，步长均匀
    ...
    def set_gather_fn(self, fn):
        klass = SigmoidContrastiveLearning if self.sigmoid_contrastive_loss else \
            partial(SoftmaxContrastiveLearning, decoupled_contrastive_learning = self.decoupled_contrastive_learning, gather_fn = fn)
        self.contrast = klass()
    ...
    @staticmethod
    @beartype
    def get_trainable_state_dict(state_dict:OrderedDict):
        trainable_state_dict = OrderedDict()
        skip_key_starts = ('audio.model', 'audio.aggregator', 'text.model')
        def is_skip_key(key):
            for skip_key_start in skip_key_starts:
                if key.startswith(skip_key_start):
                    return True
            return False
        for key, value in state_dict.items():
            if not is_skip_key(key):
    ...
    def forward(
        ...
        if return_latents: #控制这个flag，在推理时使用
        ...
        if return_pairwise_similarities:
            cosine_sim = einsum('i d, j d -> i j', audio_latents, text_latents) #相当于求点积，其中左边重复的两个d表示对该维度进行相乘，d的消失表示对该维度进行求和
            return cosine_sim
...
class MuLaNEmbedder(nn.Module):
    """
        MuLaN embedder, this module will use MuLaN to process audio or text to embeddings.
        For audio input, a sliding window will be used, and then the mean embedding will be output.
    """
    def __init__(self, mulan: MuLaN, sr = 24000, clip_secs = 10, frozen = True, checkpoint_path = None):
        """
        Args:
            mulan: MuLaN model
            sr: sampling rate of audio, set to 24000 for MuLaN model
            clip_secs: window length for audio clip in seconds, recommended 10 for MuLaN model
            frozen: whether to freeze the MuLaN model
            checkpoint_path: checkpoint path to load MuLaN model
        """
        super().__init__()
        self.mulan = mulan
        self.sr = sr
    ...
    def load(self, path):
        ...
        try:
            self.mulan.load_state_dict(pkg, strict=True)
        except:
            self.mulan.load_state_dict(pkg['model'], strict=True)
    ...
    def forward(self,
        ...
        *,
        sequential = False, #如果将sequential置为True, 则会将音频窗口线性推理, 而不会并行推理. 避免占用大量显存, 但会减慢速度.
    ):
        assert exists(wavs) ^ exists(texts), "Please provide either wavs or texts, but not both"

        if exists(wavs):
            return self._get_audio_latents(wavs = wavs, sequential = sequential)
        else:
            return self._get_text_latents(raw_texts = texts)

    def _get_audio_latents(self, wavs, *, sequential = False):
        audio_latents = []
        for wav in wavs:
            wav_tensors = []
            ...
            if sequential:
                ...
                for wav_tensor in wav_tensors:
                    audio_latent.append(self.mulan_module.get_audio_latents(wav_tensor).squeeze(0))
                    del wav_tensor
                ...
                audio_latent = self.mulan_module.get_audio_latents(wav_tensors)
                audio_latent = audio_latent.mean(dim=0)
            audio_latents.append(audio_latent)
        audio_latents = torch.stack(audio_latents, dim=0)
        return audio_latents

    def _get_text_latents(self, raw_texts):
        return self.mulan_module.get_text_latents(raw_texts=raw_texts)
