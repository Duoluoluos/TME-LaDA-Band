...
import lada_band.model.semantic_extractor.tango_descript.tools.torch_tools as torch_tools
...
def vocoder_chunk_infer(mels, vocoder, lengths=None):
    ...
    wavs = wavs.detach().float().cpu().numpy()
...
class AutoencoderKL(nn.Module):
    def __init__(
        ...
        self.latent_channel_num = ddconfig["z_channels"]
    ...
    def decode_chunk_to_waveform(self, dec):
        # import pdb; pdb.set_trace()
        if self.image_key == "fbank":
            dec = dec.squeeze(1).permute(0, 2, 1)
            # wav_reconstruction = vocoder_infer(dec, self.vocoder)
...
def build_empty_models(ckpt=None):
    config = {
        "preprocessing": {
            "audio": {
            "sampling_rate": 48000,
...
if __name__=="__main__":
    ...
    with open(json_file) as filelist:
        for fname in filelist:
            wavname = fname.strip()
            target_length = 1024*3  #10s
            print("wavname", wavname)
            mel, _, waveform = torch_tools.wav_to_fbank([wavname], target_length=target_length, fn_STFT=stft)
            mel = mel.unsqueeze(1).cuda()
            print(mel.shape)
            latents = vae.get_first_stage_encoding(vae.encode_first_stage(mel))
            print(latents.shape)
            print("111", latents.size())
