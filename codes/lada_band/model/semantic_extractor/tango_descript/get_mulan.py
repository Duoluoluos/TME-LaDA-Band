from lada_band.model.musiclm_pytorch.musiclm_pytorch import MuLaNEmbedder
import hydra
import librosa
import torch
import yaml
...
from lada_band.model.musiclm_pytorch.musiclm_pytorch import AudioSpectrogramTransformerPretrained, TextTransformerPretrained, MuLaN
from omegaconf import DictConfig
import os

_HUB_MODEL_ID_ALIASES = {
    'models--m-a-p--MERT-v1-330M': 'm-a-p/MERT-v1-330M',
    'models--xlm-roberta-base': 'FacebookAI/xlm-roberta-base',
}


def get_pretrained_config(root, name):
    if root is None:
        return _HUB_MODEL_ID_ALIASES.get(name, name)
...
def get_mulan(config):
    if not config:
        raise ValueError('A MuLaN configuration path is required.')

    with open(config, "r") as stream:
        mulan_config = yaml.safe_load(stream)
        mulan_config = Prodict.from_dict(mulan_config)

    pretraind_root = mulan_config.model.pretraind_root or os.environ.get('LADA_PRETRAINED_ROOT')
    if pretraind_root:
        pretraind_root = os.path.expanduser(pretraind_root)
        hub_root = os.path.join(pretraind_root, 'hub')
        if os.path.isdir(hub_root):
            pretraind_root = hub_root
        mulan_config.model.pretraind_root = pretraind_root
